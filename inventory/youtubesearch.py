...
class YoutubeSearch:
    ...
    def _search(self):
        ...
        url = (
            self.url + f"={encoded_search}"
        )  # f"{BASE_URL}/results?search_query={encoded_search}"
    ...
    def _parse_html(self, response, url):
        ...
        if not head:
            try:
                head = [
                    t
                    for t in data["contents"]["twoColumnBrowseResultsRenderer"]["tabs"]
                    if t.get("expandableTabRenderer", False)
                ]
