...
def create_reaction_alignment_bounds(
    ...
    if not reaction.get("unreliable_bounds", False):
        ...
        print(
            f"Creating alignment bounds with tolerance {peak_tolerance} every {seconds_per_checkpoint} seconds at {timestamps} "
        )
        ...
        for i, ts in enumerate(timestamps):
            ...
            segment_times = [
                ...
                (
                    max(0, ts - int(clip_length / 2)),
                    min(len(base_audio), ts + int(clip_length / 2)),
                ),
            ...
            segments = [
                (
                    ...
                    e,
                    base_audio[s:e],
                    song_audio_mfcc[:, round(s / hop_length) : round(e / hop_length)],
                )
                for s, e in segment_times
            ]
            ...
            for base_start, end, chunk, chunk_mfcc in segments:
                ...
                if candidates is None:
                    ...
                    print(
                        f"\tCandidates: {[ int((1000 * (c+reaction_start))/sr)/1000 for c in candidates]}  {(max(candidates) + reaction_start) / sr:.1f} [{max(candidates)/sr:.1f}]"
                    )
    ...
    if manual_bounds:
        for mbound in manual_bounds:
            ...
            ts = int(ts * sr)
            upper = int(upper * sr)
            bounds.append([ts, [upper + int(1.5 * sr) - ts - grace]])
    ...
    if reaction.get("end_reaction_search_at", False):
        bounds.append(
            [
                len(base_audio),
                [reaction.get("end_reaction_search_at") - len(base_audio) - grace],
            ]
        )
        print(
    ...
    for i, (base_ts, all_candidates, current_candidate) in enumerate(bounds):
        ...
        if len(candidates) == 0:
            ...
            print(
                f"\tFor timestamp {base_ts/sr}. Needed value below {(last_intercept+base_ts+grace)/sr}, but had min of {(min(all_candidates) + base_ts)/sr} (and max {(max(all_candidates)+base_ts)/sr})"
            )
            print("************")

            return create_reaction_alignment_bounds(
                reaction,
                first_n_samples,
                seconds_per_checkpoint=seconds_per_checkpoint + 10,
                peak_tolerance=peak_tolerance * 0.9,
            )

        if max(all_candidates) != max(candidates):
            print(
                f"New bound for {base_ts/sr} is {(max(candidates) + base_ts) / sr}, forced value below {(last_intercept+base_ts + grace)/sr}, cuts off {(max(all_candidates)+base_ts)/sr}"
            )
        else:
...
def print_alignment_bounds(reaction):
    ...
    if False:
        ...
        for x, c in alignment_bounds:
            ...
            plt.plot(
                [x_min, x / sr], [x_min + c, x / sr + c], "r--", alpha=0.5
