...
def align_by_checkpoint_probe(reaction):
    ...
    for idx,checkpoint_ts in enumerate(checkpoints): 
        ...
        if checkpoint_ts is None:
            ...
            starting_points = thin_at_checkpoint( reaction,
                                                  past_the_checkpoint = past_the_checkpoint, 
                                                  checkpoint_idx = idx,
                                                  checkpoint_ts = checkpoint_ts, 
                                                  checkpoints = checkpoints, 
                                                  prunes_all = prunes_all,
                                                  enable_timely_evaluation_skipping = idx < len(checkpoints) - 1 and checkpoints[idx + 1] is not None,
...
def thin_at_checkpoint(   reaction, 
                          past_the_checkpoint, 
                          checkpoint_idx,
                          checkpoint_ts, 
                          checkpoints, 
    ...
    checkpoint_threshold_tight = checkpoint_threshold_tight_diff = .9 
    checkpoint_threshold_light = .25 + .6 * (checkpoint_ts / song_length)    
    checkpoint_threshold_light_diff = .5  
    ...
    for candidate in vetted_candidates:
        ...
        passes_time = enable_timely_evaluation_skipping and path[-1][3] and path[-1][3] > checkpoints[checkpoint_idx + 1]
        if not passes_time:
            num_to_evaluate += 1
    ...
    for location, candidates in all_by_current_location.items():
        if len(candidates) < 2:
            ...
            for candidate, reaction_end, score, truncated_path in candidates:
                ...
                passes_time = enable_timely_evaluation_skipping and relative_current_end and relative_current_end > checkpoints[checkpoint_idx + 1]
    ...
    if plot_candidate_paths and len(candidates_to_plot) > 0:
        visualize_candidate_paths(reaction, checkpoint_idx, checkpoint_ts, candidates_to_plot, best_score_past_checkpoint, scoring_function=get_score_for_overall_comparison, done=checkpoint_idx==len(checkpoints) - 2, show_each_iteration=False)
