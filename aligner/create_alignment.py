...
def create_alignment_for_reaction_video(reaction, force=False):
    global conf
    ...
    if not os.path.exists(alignment_metadata_file):
        ...
        if image_based_reaction_start is None:
            reaction["start_reaction_search_at"] = reaction.get("start_reaction_search_at", 3 * sr)
...
def write_aligned_reaction_video(reaction, extend_by=0):
    alignment_metadata_file = reaction.get("alignment_metadata")
    assert os.path.exists(alignment_metadata_file)
    metadata = read_object_from_file(alignment_metadata_file)
