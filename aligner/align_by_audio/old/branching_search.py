...
def branching_search(
    reaction,
    current_path=None,
    current_path_checkpoint_scores=None,
    current_start=0,
    reaction_start=0,
    continuations=None,
    greedy_branching=False,
    recursive=True,
    probe_to_time=False,
    peak_tolerance=None,
):
    ...
    if probe_to_time and current_start >= probe_to_time:
        continuations.append(
            [current_path, current_path_checkpoint_scores, current_start, reaction_start]
        )
        return [None]
    ...
    try:
        ...
        reaction_audio_depleted = reaction_start >= len(reaction_audio) - 1 or current_start > len(
            base_audio
        ) - int(0.5 * sr)
        if matched_all_of_base or reaction_audio_depleted:
            ...
            if reaction_audio_depleted:
                ...
                filler_segment = (
                    reaction_start,
                    reaction_start + length_remaining,
                    current_start,
                    current_start + length_remaining,
                    True,
                )
        ...
        candidate_starts = find_segment_starts(
            ...
            closed_chunk=chunk,
            closed_chunk_mfcc=song_audio_mfcc[
        ...
        if candidate_starts is None or len(candidate_starts) == 0:
            increment = int(sr * 0.25)
            current_start += increment

            if (len(base_audio) - step - current_start) / sr < 0.5:
                ...
                result = branching_search(
                    reaction,
                    current_path,
                    copy.copy(current_path_checkpoint_scores),
                    current_start,
                    reaction_start,
        ...
        for candidate_segment_start in candidate_starts:
            ...
            starting_points.append(
                (candidate_segment_start, current_start, copy.deepcopy(current_path))
            )

            adjusted_start = check_for_start_adjustment(
            ...
            if adjusted_start is not None and adjusted_start > 0:  # adjusted_start > sr / 100:
                ...
                starting_points.append(
                    (
                        candidate_segment_start + adjusted_start,
                        current_start + adjusted_start,
                        my_adjusted_path,
                    )
                )
        ...
        for ci, (candidate_segment_start, next_start, my_path) in enumerate(starting_points):
            ...
            while find_end:
                segment, next_start, next_reaction_start = find_segment_end(
                    reaction,
                    next_start,
                    next_reaction_start,
            ...
            filler = segment[-1]
            if (not recursive and filler and len(my_path) < 10) or (
                recursive
                and (not greedy_branching or ci < greedy_branching)
                and (not probe_to_time or next_start < probe_to_time)
            ):
                # print('recursing', filler, my_path)
                continued_paths = branching_search(
                    reaction,
...
def append_or_extend_segment(my_path, segment):
    if len(my_path) == 0:
        ...
        return
    (
        previous_reaction_start,
        previous_reaction_end,
        previous_start,
        previous_end,
        previous_is_filler,
    ) = my_path[-1]
    ...
    if (
        new_reaction_start - previous_reaction_end > 1
        or new_is_filler
        or new_is_filler != previous_is_filler
    ):
        my_path.append(segment)
    else:
