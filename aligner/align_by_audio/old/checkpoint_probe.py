...
from aligner.pruning_search import print_prune_data, is_path_quality_poor, prune_types
from aligner.scoring_and_similarity import print_path, calculate_partial_score
from aligner.branching_search import branching_search


def initialize_prune_tracking():
    prunes_all = {
        "tight": 0,
        "light": 0,
        "location": 0,
...
def align_by_checkpoint_probe(reaction):
    ...
    starting_points = []
    new_paths = branching_search(
        reaction,
        current_path=[],
        current_path_checkpoint_scores={},
        current_start=0,
        reaction_start=10
        * sr,  # reactions don't start at the beginning, but sometimes they have a song preview we want to skip
        continuations=starting_points,
        recursive=False,
        peak_tolerance=conf.get("peak_tolerance") * 0.25,
    )
    ...
    for idx, checkpoint_ts in enumerate(checkpoints):
        ...
        for idx2, starting_point in enumerate(starting_points):
            ...
            print(
                f"\t[{idx / len(checkpoints)*100:.1f}% / {100 * idx2 / len(starting_points):.1f}%] Reaction: {reaction_start / sr:.8f} Base: {current_start / sr:.4f} ... {len(past_the_checkpoint)} continuations",
                end="\r",
            )
            ...
            if checkpoint_ts and current_start >= checkpoint_ts:
                ...
                new_paths = branching_search(
                    reaction,
                    current_path=path,
                    current_path_checkpoint_scores=checkpoint_scores,
                    current_start=current_start,
                    reaction_start=reaction_start,
                    continuations=continuations,
                    recursive=True,
                    probe_to_time=checkpoint_ts,
                )
                update_paths(reaction, paths, new_paths)
        ...
        if checkpoint_ts is None:
            ...
            starting_points = thin_at_checkpoint(
                reaction,
                past_the_checkpoint=past_the_checkpoint,
                checkpoint_idx=idx,
                checkpoint_ts=checkpoint_ts,
                checkpoints=checkpoints,
                prunes_all=prunes_all,
                enable_timely_evaluation_skipping=idx < len(checkpoints) - 1
                and checkpoints[idx + 1] is not None,
                plot_relationships=plot_relationships,
                plot_candidate_paths=plot_candidate_paths,
            )
...
def thin_at_checkpoint(
    ...
    checkpoints,
    prunes_all=None,
    enable_timely_evaluation_skipping=None,
    plot_relationships=False,
    plot_candidate_paths=False,
):
    if enable_timely_evaluation_skipping is None:
        enable_timely_evaluation_skipping = checkpoint_idx < len(checkpoints) - 1
    ...
    vetted_candidates = [
        c
        for c in past_the_checkpoint
        if checkpoint_ts > 0.95 * song_length or not is_path_quality_poor(reaction, c[0])
    ]
    ...
    # Final ones for cross-expander algorithm
    checkpoint_threshold_tight = checkpoint_threshold_tight_diff = 0.9
    checkpoint_threshold_light = 0.25 + 0.6 * (checkpoint_ts / song_length)
    checkpoint_threshold_light_diff = 0.5
    checkpoint_threshold_location = 0.95
    ...
    for candidate in vetted_candidates:
        ...
        reaction_end, score_at_checkpoint, truncated_path = calculate_partial_score(
            reaction, path, end=checkpoint_ts, start=start_at
        )
        ...
        passes_time = (
            enable_timely_evaluation_skipping
            and path[-1][3]
            and path[-1][3] > checkpoints[checkpoint_idx + 1]
        )
        if not passes_time:
            num_to_evaluate += 1
    ...
    )  # sort by reaction_end, with a tie breaker of -overall score.
    ...
    for expanded_candidate in candidates:
        ...
        passes_tight = passes_time or checkpoint_threshold_tight <= get_score_ratio_at_checkpoint(
            reaction,
            score_at_checkpoint,
            best_score_to_point,
            get_score_for_tight_bound,
            checkpoint_ts,
        )
        passes_mfcc = passes_time or checkpoint_threshold_light <= get_score_ratio_at_checkpoint(
            reaction,
            score_at_checkpoint,
            best_score_past_checkpoint,
            get_score_for_overall_comparison,
            checkpoint_ts,
        )
        ...
        if passes_tight and passes_mfcc:
            passes_tight_diff = (
                passes_time
                or checkpoint_threshold_tight_diff
                <= get_score_ratio_at_checkpoint_diff(
                    checkpoints_reversed,
                    reaction,
                    candidate,
                    best_candidate_to_point,
                    get_score_for_tight_bound,
                    checkpoint_ts,
                )
            )
            passes_mfcc_diff = (
                passes_time
                or checkpoint_threshold_light_diff
                <= get_score_ratio_at_checkpoint_diff(
                    checkpoints_reversed,
                    reaction,
                    candidate,
                    best_candidate_past_the_checkpoint,
                    get_score_for_overall_comparison,
                    checkpoint_ts,
                )
            )
        ...
        if plot_relationships:
            ideal_filter = passes_time or (
                passes_tight and passes_mfcc and passes_tight_diff and passes_mfcc_diff
            )
    ...
    for location, candidates in all_by_current_location.items():
        if len(candidates) < 2:
            ...
            for candidate, reaction_end, score, truncated_path in candidates:
                ...
                passes_time = (
                    enable_timely_evaluation_skipping
                    and relative_current_end
                    and relative_current_end > checkpoints[checkpoint_idx + 1]
                )
                passes_location = (
                    get_score_for_location(score) / best_at_location
                    >= checkpoint_threshold_location
                )
                if passes_time or passes_location:
                    passes_location_diff = (
                        checkpoint_threshold_location
                        <= get_score_ratio_at_checkpoint_diff(
                            checkpoints_reversed,
                            reaction,
                            candidate,
                            best_candidate_at_location,
                            get_score_for_location,
                            checkpoint_ts,
                        )
                    )
    ...
    if len(past_the_checkpoint) > 0:
        print(
            f"\tKept {len(best_past_the_checkpoint)} of {len(past_the_checkpoint)} ({100 * len(best_past_the_checkpoint) / len(past_the_checkpoint):.1f}%)"
        )
    ...
    if plot_candidate_paths and len(candidates_to_plot) > 0:
        visualize_candidate_paths(
            reaction,
            checkpoint_idx,
            checkpoint_ts,
            candidates_to_plot,
            best_score_past_checkpoint,
            scoring_function=get_score_for_overall_comparison,
            done=checkpoint_idx == len(checkpoints) - 2,
            show_each_iteration=False,
        )

    return best_past_the_checkpoint
...
def get_score_ratio_at_checkpoint_diff(
    checkpoints_reversed, reaction, candidate, best_candidate, scoring_function, checkpoint
):
    ...
    for past_checkpoint in checkpoints_reversed:
        if (checkpoint - past_checkpoint) > 6 * sr and abs(
            1
            - scoring_function(checkpoint_scores[past_checkpoint])
            / scoring_function(best_checkpoint_scores[past_checkpoint])
        ) < 0.01:
...
def visualize_candidate_paths(
    ...
    for path, score in paths:
        alpha = min(1, 0.2 + 0.8 * scoring_function(score) / scoring_function(best_score))
...
def truncate_path(current_path, end, start=0):
    ...
    if key not in truncated_path_cache:
        ...
        for segment in current_path:
            ...
            if current_end >= end:
                ...
                modified_path.append(
                    (reaction_start, reaction_end, current_start, current_end, filler)
                )
        ...
        if adjusted_reaction_end is None:
            raise Exception(
                f"WEIRD! Could not calculate partial score for {checkpoint_ts} {current_path}"
            )
            ...
            for segment in back_adjusted_path:
                ...
                if current_end > start:
                    if current_start < start:
                        modified_path.append(
                            (
                                reaction_start + (start - current_start),
                                reaction_end,
                                start,
                                current_end,
                                filler,
