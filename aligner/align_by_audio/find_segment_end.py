...
def find_correlation_end(
    ...
    while current_end < len(base_audio) and reaction_end < end_at:
        ...
        segment_scores.append(
            (reaction_start, reaction_end, chunk_score, current_start, current_end)
        )
        ...
        if slope <= neg_slope_avg:
            ...
            while i > 0:
                ...
                if last_neg_slopes[i][0] <= slope / (2 * window):
                    next_end_at = max(
                        reaction_start, min(end_at, last_neg_slopes[i][1][1] + step * window)
                    )
    ...
    if step > 250:
        ...
        while (
            (current_end - current_start) % samples_per_frame() > 0
            and reaction_end < len(reaction_audio)
            and current_end < len(base_audio)
        ):
            current_end += 1
            reaction_end += 1

        result = (
            Decimal(current_end - current_start) / Decimal(sr) * Decimal(universal_frame_rate())
        )
...
def check_for_start_adjustment(
    reaction, current_start, reaction_start, candidate_segment_start, current_chunk_size
):
    base_audio = conf.get("song_audio_data")
    reverse_search_bound = conf.get("reverse_search_bound")

    open_end = min(
        current_start + current_chunk_size + int(reverse_search_bound * sr), len(base_audio)
    )
    ...
    candidate_reaction_chunk = reaction_audio[
        candidate_reaction_chunk_start:candidate_reaction_chunk_end
    ]
    ...
    reverse_index = find_segment_starts(
        ...
        open_chunk_mfcc=song_audio_mfcc[
            :, round(current_start / hop_length) : round(open_end / hop_length)
        ],
        closed_chunk=candidate_reaction_chunk,
        closed_chunk_mfcc=reaction_audio_mfcc[
            :,
            round(candidate_reaction_chunk_start / hop_length) : round(
                candidate_reaction_chunk_end / hop_length
            ),
        ],
    ...
    if reverse_index and reverse_index > 0:  # sr / 100:
        ...
        # seek to a frame boundary
        while (
            (reverse_index - current_start) % samples_per_frame() > 0
            and reverse_index < len(reaction_audio)
            and current_start + reverse_index < len(base_audio)
        ):
            reverse_index += 1
...
def find_segment_end(
    ...
    scope_key = (
        f"({current_start}, {reaction_start + candidate_segment_start}, {current_chunk_size})"
    )
    ...
    n_samples = int(conf.get("step_size") * sr)
    ...
    candidate_current_end, candidate_reaction_end = find_correlation_end(
        reaction, current_start, reaction_start, step=n_samples, cache={}
    )
