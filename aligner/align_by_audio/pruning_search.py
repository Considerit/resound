...
def is_path_quality_poor(reaction, path):
    ...
    if len(path_without_fillers) >= 3:
        ...
        if not first[-1] and not middle[-1] and not last[-1]:  # ignore fillers
            ...
            if (first[1] - first[0]) + (middle[1] - middle[0]) + (
                last[1] - last[0]
            ) < 3 * spacing_min_length:
                if (
                    middle[0] - first[1] > spacing_max_separation
                    and last[0] - middle[1] > spacing_max_separation
                ):
                    return True
    ...
    for i, segment in enumerate(path):
        ...
        if (
            i == 0
            and len(path) > 1
            and not filler_after
            and space_after > 60 * sr
            and (reaction_end - reaction_start) < 3 * sr
        ):
            return True
        ...
        if (
            ...
            and len(path) > 2
            and not filler_after
            and space_after > 0.1 * sr
            and path[i + 2][0] - path[i + 1][1] > 0.1 * sr
            and ((reaction_end - reaction_start) + (path[1][1] - path[1][0])) < 6 * sr
        ):
            return True

        if (not filler_before and 0.1 * sr < space_before < sr) or (
            not filler_after and 0.1 * sr < space_after < sr
        ):
            return True
