...
def apply_compression(audio_data, threshold_db=-30.0, ratio=2.0):
    ...
    audio_data_db = librosa.amplitude_to_db(np.abs(audio_data))

    # Apply compression
    compressed_audio_db = np.where(
        audio_data_db > threshold_db,
        threshold_db + (audio_data_db - threshold_db) / ratio,
        audio_data_db,
    )
...
def find_normalization_factor(song_data, reaction_data, ref_tuple, search_window=None):
    ...
    if sum_amplitude_reaction == 0:
        raise ValueError(
            "The sum of absolute amplitudes in the reaction data segment is zero. Cannot normalize."
        )
...
def normalize_audio_manually(song_data, reaction_data, ref_tuple, search_window=None):
    ...
    normalization_factor = max(0.9, normalization_factor)
...
def get_highest_amplitude_segments(audio, segment_length):
    ...
    amplitude_sums = np.array(
        [
            np.sum(np.abs(audio[i : i + window_length]))
            for i in range(0, len(audio) - window_length, step_size)
        ]
    )
...
def best_matching_segment(
    song_segment, reaction_audio, song_mfcc_segment, reaction_mfcc, hop_length
):
...
def normalize_reaction_audio(
    ...
    segment_length=1,
    top_n=10,
    shot_in_the_dark=False,
    good_match_threshold=0.9,
):
    from utilities import conf
    ...
    if len(top_matches) > 0:
        ...
        for match in top_matches:
            ...
            normalization_factor = find_normalization_factor(
                song_audio, reaction_audio, (ref_tuple[0] / sr, ref_tuple[1] / sr)
            )
        ...
        normalization_factor = avg(
            [avg(normalization_factors), statistics.median(normalization_factors)]
        )  # midmean
        normalization_factor = max(0.9, min(4, normalization_factor))

        print(
            f"\t\tDone! normalization_factor={normalization_factor}",
            normalization_factors,
        )
        ...
        if top_n < 100:
            next_top_n = top_n * 5
            return normalize_reaction_audio(
                reaction,
                song_audio,
                reaction_audio,
                song_mfcc,
                ...
                segment_length=1,
                top_n=next_top_n,
                shot_in_the_dark=shot_in_the_dark,
                good_match_threshold=good_match_threshold,
            )

        elif not shot_in_the_dark:
            normalized_reaction_data = (
                reaction_audio * 5
            )  # Almost always failure to find an alignment point is when the reaction audio is way low.
            # We'll try to give it a boost and then try again.

            normalized_reaction_mfcc = librosa.feature.mfcc(
                y=normalized_reaction_data,
                sr=sr,
            ...
            return normalize_reaction_audio(
                ...
                normalized_reaction_data,
                song_mfcc,
                normalized_reaction_mfcc,
                segment_length=1,
                top_n=10,
                shot_in_the_dark=True,
...
def calculate_perceptual_loudness(audio, frame_length=2048, hop_length=None):
    ...
    rms = np.array(
        [
            np.sqrt(np.mean(np.square(audio[i : i + frame_length])))
...
def adaptive_pitch_matching(song, reaction, segment_length):
    ...
    for i in range(num_segments):
        ...
        reaction_segments[i] = librosa.effects.pitch_shift(
            reaction_segments[i], sr=sr, n_steps=(pitch_song - pitch_reaction)
        )
    ...
    if len(reaction) > len(pitch_matched_reaction):
        pitch_matched_reaction = np.append(
            pitch_matched_reaction, reaction[len(pitch_matched_reaction) :]
        )
...
def equalize_spectra(audio1, audio2):
    ...
    mask = np.abs(fft_audio1) / np.abs(
        fft_audio2 + 1e-10
    )  # The small number prevents division by zero
