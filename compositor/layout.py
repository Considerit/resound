...
def create_layout_for_composition(base_video, width, height, shape="hexagon"):
    ...
    if include_base_video:
        ...
        if base_video.audio.fps != conversion_audio_sample_rate:
            base_video = base_video.set_audio(
                base_video.audio.set_fps(conversion_audio_sample_rate)
            )

    key = f"{total_videos}-{base_video_proportion}-{include_base_video}-{base_video_placement}".replace(
        "/", "x"
    )
    ...
    if os.path.exists(output_path):
        ...
        hex_grid, cell_size = generate_grid(
            width, height, total_videos, bounds, center, shape=shape
        )

        hex_grid = sorted(
            hex_grid, key=lambda cell: distance_from_region(cell, bounds), reverse=False
        )
...
def apply_adjustments(layout):
    ...
    if grid_adjustment is not None:
        layout["seats"] = (
            [seat[0] + grid_adjustment[0], seat[1] + grid_adjustment[1]] for seat in layout["seats"]
        )

        new_assignments = {}
        for assignment in layout["assignments"].items():
            new_assignments[assignment[0]] = [
                assignment[1][0] + grid_adjustment[0],
                assignment[1][1] + grid_adjustment[1],
            ]
        ...
        for reactor in reaction["reactors"]:
            ...
            if (horiz_gaze == "left" and reactor["grid_assignment"][0] < base_width / 2) or (
                ...
                if reactor["layout_adjustments"].get("flip-x", True):
                    print(
                        "\t\tAuto flipping", reactor["key"], reactor["grid_assignment"], base_width
                    )
    ...
    for k, v in base_video_adjustments.items():
        if k == "shift":
            layout["base_location"] = (
                layout["base_location"][0] + v[0],
                layout["base_location"][1] + v[1],
            )
            layout["base_video_centroid"] = (
                layout["base_video_centroid"][0] + v[0],
                layout["base_video_centroid"][1] + v[1],
            )
...
def generate_grid(width, height, min_cells, outside_bounds=None, center=None, shape="hexagon"):
    ...
    while len(coords) < min_cells:
        ...
        if shape == "hexagon" or shape == "circle":
            dy = math.ceil(0.75 * 2 * a) - 1  # np.sqrt(3) * a
        elif shape == "diamond":
            dy = math.ceil(0.5 * 2 * a) - 1  # np.sqrt(3) * a
    ...
    print(
        f"\tLayout: len(hex_grid) = {len(coords)}  target_cells={min_cells} cell_size={cell_size}"
    )

    return coords, cell_size  # return 2 * a (diameter) to match with the circle representation


def get_seat_preferences_for_gaze_direction(
...
def get_seat_preferences(
    reaction, grid_centroid, grid_size, all_seats, base_video_width, base_video_height
):
    ...
    if key not in seat_preferences:
        ...
        preference = get_seat_preferences_for_gaze_direction(
            horizontal_orientation,
            vertical_orientation,
            base_video_width,
            base_video_height,
        ...
        if horizontal_orientation != "center":
            ...
            flipped_preference = get_seat_preferences_for_gaze_direction(
...
def seat_score(reaction, seat, grid_centroid, max_distance, seat_preference, base_size):
    ...
    for i, (ideal_seat, section_preference) in enumerate(seat_preference):
        ...
        section_score = (
            max_distance - distance_from_region(seat, section_preference)
        ) / max_distance

        section_score *= 1 - 0.05 * i
...
def assign_seats_to_reactors(
    seats, grid_centroid, seat_size, base_video_width, base_video_height, grid_size
):
    ...
    total_reactors = 0
    for channel, reaction in conf.get("reactions").items():
        seat_preferences[channel] = get_seat_preferences(
            reaction, grid_centroid, grid_size, seats, base_video_width, base_video_height
        )
    ...
    for seat, same_row, adjacents in seats_with_adjacents:
        ...
        for channel, reaction in conf.get("reactions").items():
            ...
            seat_scores[seat_key][channel] = seat_score(
                reaction,
                seat,
                grid_centroid,
                max_distance,
                seat_preference=seat_preference,
                base_size=(base_video_width, base_video_height),
            )
    ...
    def assign_seats(chosen_seats, chosen_channel):
        ...
        if len(reactors) > len(chosen_seats):
            ...
            while len(chosen_seats) < len(reactors):
                for seat in seats:
                    seat_key = str(seat)
                    if seat_key not in seating_by_seat and seat_key not in [
                        str(t) for t in chosen_seats
                    ]:
        ...
        for i, reactor in enumerate(reactors):
            ...
            print(
                f"\t\tASSIGNED {chosen_seat} to {chosen_channel} / {i}. Priority={reaction.get('priority')} Target position={seat_preferences[chosen_channel]}"
            )
    ...
    while len(seating_by_reactor.keys()) < total_reactors:
        ...
        if len(seat_scores.keys()) == 0:
            raise Exception(
                f"We've seated {len(seating_by_reactor.keys())} of {total_reactors} reactors, but don't seem to have any more available."
            )
        ...
        for seat, same_row, adjacents in seats_with_adjacents:
            ...
            for channel, score in seat_scores[seat_key].items():
                ...
                if num_reactors > 1:
                    ...
                    if len(group_seats) < num_reactors:
                        score *= 0.5 * num_reactors / len(group_seats)
                ...
                if score > highest_score or (
                    score == highest_score and priority > highest_priority
                ):
...
def generate_irregular_hex_layout(ax, aspect_ratio=16 / 9, hex_size=0.1, rows=10, cols=10):
    ...
    for x, y, col, row in grid:
        ...
        if (col, row) not in perturbations:
            perturbations[(col, row)] = [
                np.random.rand() * hex_size * 0.3 - hex_size * 0.15 for _ in range(6)
            ]
...
def plot_hexagon(ax, center, size, edge_color, perturbations):
    ...
    hexagon = patches.Polygon(
        create_perturbed_hexagon(center, size, perturbations),
        closed=True,
        fill=False,
...
def generate_voronoi_diagram(width, height, num_points, num_large_cells, iterations=5):
    ...
    large_cell_points = np.random.rand(num_large_cells, 2) * [width * 0.2, height * 0.2] + [
        width * 0.4,
        height * 0.4,
    ]
...
if __name__ == "__main__":
    ...
    circle_centroids = np.array(
        [[0.2, 0.5], [0.4, 0.7], [0.6, 0.3], [0.8, 0.5]]
