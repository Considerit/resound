...
def new_init(self, *args, **kwargs):
    ...
    self.my_position = (
        0,
        0,
    )  # Default position could be (0,0) or any other default you prefer
...
def create_reaction_concert(extend_by=0, output_size=(3840, 2160), shape="hexagon"):
    ...
    video_background = get_video_background(
        output_size, clip_length=base_video.duration + extend_by
    )

    base_video, cell_size, base_video_position = create_layout_for_composition(
        base_video, width, height, shape=shape
    )
    ...
    all_clips, clip_length, video_background = create_video_clips(
        base_video,
        video_background,
        cell_size,
        draft,
    ...
    if base_video_position is not None:
        base_video = all_clips[-1]["video"] = all_clips[-1]["video"].set_position(
            base_video_position
        )
        print(
            f"\t\tSetting position of base video {id(base_video)} in composition to ",
            base_video_position,
        )
    ...
    for idx, tile in enumerate(tiles):
        ...
        if draft:
            ...
            if not os.path.exists(tile_path):
                tile.set_fps(15).write_videofile(
                    tile_path,
            ...
            tile.write_videofile(
                tile_path, codec="libx264", preset="slow", ffmpeg_params=["-crf", "18"]
            )

        if idx == 0:
            merge_audio_and_video(tile_path, audio_output)
...
def merge_audio_and_video(output_path, audio_output):
    ...
    command = ["mv", output_path + ".mp4", output_path]
    subprocess.run(command)
...
def create_channel_labels_video(active_segments, cell_size, output_size):
    ...
    for channel, start, end in active_segments:
        ...
        shadow_clip = TextClip(
            reaction.get("channel_label"), fontsize=28 * 2, color="black", font=font
        )
        # Create shadow text clip (black and slightly offset)
        shadow_clip2 = TextClip(
            reaction.get("channel_label"), fontsize=28 * 2, color="black", font=font
        )

        # Create main text clip (white)
        txt_clip = TextClip(
            reaction.get("channel_label"), fontsize=28 * 2, color="white", font=font
        )
        ...
        bar_position = (x, y + int(text_height * 0.75) + progress_bar_height + 4)
...
def create_video_clips(
    ...
    base_clip = {"base": True, "priority": 0, "video": base_video}
...
def create_audio_clips(base_video, output_size):
    ...
    for i, (name, reaction) in enumerate(conf.get("reactions").items()):
        ...
        reaction["audio_volume"] = get_audio_volume(
            audio, downsample_factor=downsample_factor
        )  # I can probably downsample this pretty heavily and still have it accurate enough
...
def create_border_clip_old(
    ...
    def make_border_frame(t):
        ...
        in_foreground = (
            len(audible_segments) > 0
            and idx <= audible_segments[0][1]
            and audible_segments[0][2] > 0.2
        )
...
def create_border_clip(
    ...
    for segment in audible_segments:
        ...
        if segment[2] > 0.2 and duration > 1.5:
